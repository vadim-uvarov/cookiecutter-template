# Placeholder preprod stack. It provisions nothing yet — `terraform apply` just prints a
...
terraform {
  ...
  #   terraform -chdir=terraform/preprod init \
...
resource "null_resource" "deploy_message" {
  provisioner "local-exec" {
    command = "echo 'Deploying {{ cookiecutter.project_name }} to PREPROD (placeholder — nothing provisioned yet).'"
  }
